...
def list_notifications(db: Session = Depends(get_db), user: User = Depends(require_admin)):
...
def unread_count(db: Session = Depends(get_db), user: User = Depends(require_admin)):
...
def mark_read(notification_id: UUID, db: Session = Depends(get_db), user: User = Depends(require_admin)):
