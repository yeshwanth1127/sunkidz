...
from app.services.marks_card import card_recency_key, save_marks_signature
...
def get_my_children_marks_cards(
    ...
    """Marks cards sent to the parent for their linked children.

    Returns exactly one card per (child, academic year): the single latest
    version. Older / superseded rows are never returned, no matter how many
    times a card was updated and re-sent.
    """
    links = db.query(ParentStudentLink).filter(ParentStudentLink.user_id == user.id).all()
    student_ids = [l.student_id for l in links]
    if not student_ids:
    ...
    # Collapse to the single most-recent card per (student, academic_year).
    latest_by_key: dict = {}
    for c in sent_cards:
        key = (c.student_id, c.academic_year)
        if key not in latest_by_key or card_recency_key(c) > card_recency_key(
            latest_by_key[key]
        ):
            latest_by_key[key] = c
    cards = sorted(
        latest_by_key.values(),
        key=lambda c: c.sent_to_parent_at,
        reverse=True,
    )
    result = []
    for card in cards:
        ...
        if s.class_id:
            c = db.query(Class).filter(Class.id == s.class_id).first()
            class_name = c.name if c else None
        # Fallback name from the existing linked-parent account, mirroring the
        # admin/teacher student endpoints — used by the card when the student
        # record has no explicit father/mother name.
        linked_parent_name = None
        if s.parent_links:
            try:
                linked_parent_name = s.parent_links[0].user.full_name
            except Exception:
                linked_parent_name = None
        result.append({
            "id": str(card.id),
            ...
            "parent_name": linked_parent_name or s.father_name or s.mother_name,
            "date_of_birth": s.date_of_birth.isoformat() if s.date_of_birth else None,
...
@router.post("/marks-cards/{student_id}/signature")
async def upload_parent_marks_signature(
    student_id: UUID,
    ...
    """A parent uploads / replaces the Parent signature image on their own
    child's current marks card. The role is fixed to 'parent' — a parent can
    never write the Class Teacher or Principal signature."""
    ...
    return save_marks_signature(
        db, student_id, academic_year, "parent", file.filename, content
    )
