...
def _ensure_branch_classes(db: Session, branch_id: UUID, system_type: str, old_system_type: str | None = None) -> None:
    ...
    # Normalize aliases (for example IG-1 -> IG1) without creating duplicate names.
    for cls in existing:
...
def list_classes(
    ...
    classes = sorted(q.all(), key=lambda c: grade_sort_key(c.name))
...
def create_class(
    ...
    if not branch:
        raise HTTPException(status_code=404, detail="Branch not found")
    if is_legacy_class_name(data.name):
        raise HTTPException(
            status_code=400,
            detail="Nursery/LKG/UKG are no longer valid classes. Use Playgroup, IG1, IG2 or IG3.",
        )
...
def shift_student_branch(
    ...
    if class_id_raw:
        ...
        # Keep assignment valid by defaulting to the canonically-first class
        # (Playgroup, else IG1, ...) of the selected branch when available.
        branch_classes = db.query(Class).filter(Class.branch_id == branch_id).all()
        cls = min(branch_classes, key=lambda c: grade_sort_key(c.name)) if branch_classes else None
...
def upsert_student_fees(
    ...
    def _amount(val, field: str) -> float:
        """Parse an amount exactly as sent. Reject garbage with 400 rather than
        silently substituting a default."""
        try:
            f = float(val)
        except (TypeError, ValueError):
            raise HTTPException(status_code=400, detail=f"Invalid amount for {field}")
        if f < 0:
            raise HTTPException(status_code=400, detail=f"{field} cannot be negative")
        return f

    # Only touch a standard field when the admin actually sent it. Never reset an
    # untouched field to 0.
    for field in ("advance_fees", "term_fee_1", "term_fee_2", "term_fee_3"):
        if field in body and body[field] is not None:
            setattr(fee_structure, field, _amount(body[field], field))
        elif getattr(fee_structure, field) is None:
            setattr(fee_structure, field, 0.0)
    ...
    # Persist custom fields when provided. Store exactly what the admin entered;
    # reject malformed rows instead of silently dropping them.
    if "custom_fields" in body:
        ...
        for cf in raw_cf:
            ...
            # A completely empty row is ignored; a partial one is an error.
            if not key and not label and cf.get("amount") in (None, "", 0, 0.0):
                continue
            if not label:
                raise HTTPException(status_code=400, detail="Custom fee is missing a name")
            if not _re.match(r'^[a-z0-9_]+$', key):
                raise HTTPException(status_code=400, detail=f"Invalid key for custom fee '{label}'")
            if key in reserved:
                raise HTTPException(status_code=400, detail=f"'{label}' uses a reserved key")
            if key in seen_keys:
                raise HTTPException(status_code=400, detail=f"Duplicate custom fee '{label}'")
            seen_keys.add(key)
            validated.append({"key": key, "label": label, "amount": _amount(cf.get("amount", 0), f"custom fee '{label}'")})
...
def get_analytics(
    ...
    def _custom_fields_total(fs: FeeStructure) -> float:
        if not fs.custom_fields_json:
            return 0.0
        try:
            return sum(float(cf.get("amount", 0)) for cf in _json.loads(fs.custom_fields_json))
        except Exception:
            return 0.0
