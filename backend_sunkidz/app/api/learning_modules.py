...
from app.services.google_sheets_client import SheetConfigError
...
async def upload_video(
    ...
    if school_day is not None:
        if school_day < 1 or school_day > 400:
            raise HTTPException(status_code=400, detail="school_day must be between 1 and 400")
...
def _get_holiday_dates(db: Session, start_year: int, branch_id: Optional[UUID] = None) -> set:
    """Admin-marked holiday dates for an academic year, optionally branch-scoped.
    Same source/shape as `syllabus.py`'s `_get_holiday_dates` and `almanac.py`'s
    `_holiday_dates_for_branch` -- Learning Modules' own local, independent
    day calendar (NOT the Google-Sheet-driven school calendar)."""
...
def get_class_calendar(
    ...
    """Day 1-180 working calendar for a class -- Learning Modules' own local,
    independent calendar (June 1 start, admin-marked holidays via
    SyllabusHoliday), same data source `syllabus.py`'s calendar uses. This is
    deliberately NOT the Google-Sheet-driven school calendar
    (`school_calendar_service`) -- that stays a separate feature, still
    available via POST /learning-modules/calendar/refresh, just not read here.
    Videos and folders stay keyed by school day number.
    """
    ...
    cal_days = get_school_days_with_dates(start_year, holiday_dates)
    ...
    # Days that have at least one uploaded video inside a subject folder
    # (the DayFolder/DayFolderContent flow behind the per-subject upload UI
    # reachable from this calendar) -- distinct from the school_day-tagged
    # LearningVideo rows above, which come from the separate class-upload/
    # branch-upload flow. The calendar's "video uploaded" state must
    # reflect either source so it updates immediately after an upload.
    folder_video_days_raw = (
        db.query(DayFolder.school_day)
        .join(DayFolderContent, DayFolderContent.folder_id == DayFolder.id)
        .filter(
            DayFolder.class_id == str(class_id),
            DayFolder.academic_year_start == ay_start,
            DayFolderContent.content_type == "video",
        )
        .distinct()
        .all()
    )
    folder_video_days = {row[0] for row in folder_video_days_raw}

    result_days = []
    for cd in cal_days:
        ld = cd["day"]
        cd_date = date.fromisoformat(cd["date"])
        result_days.append({
            "day": ld,
            "learning_day": ld,
            "date": cd["date"],
            "weekday": cd_date.strftime("%A"),
            "is_working_day": True,
            "label": None,
            "category": "working",
            "school_event": None,
            ...
            "has_video": bool(videos_by_day.get(ld)) or ld in folder_video_days,
        })

    return {
        "academic_year_start": ay_start.isoformat(),
        "academic_year_str": f"{start_year}-{str(start_year+1)[2:]}",
        "working_day_count": len(cal_days),
        "days": result_days,
    }


@router.post("/calendar/refresh")
def refresh_school_calendar(
    user: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Force an immediate re-pull of the school calendar from Google Sheets."""
    try:
        return school_calendar_service.sync(db, force=True)
