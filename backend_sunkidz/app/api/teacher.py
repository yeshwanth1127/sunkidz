...
def get_marks(
    ...
    """Get the current marks card for a student in teacher's class (latest version)."""
    class_id = _teacher_class_id(user, db)
    if not _student_in_teacher_class(student_id, class_id, db):
        raise HTTPException(status_code=404, detail="Student not found in your class")
...
def send_marks_to_parent(
    ...
    """Send the current marks card to the parent (latest saved version).

    Operates on the single latest row and stamps it as sent; older duplicate
    rows (if any) are pruned so the parent never sees a superseded copy.
    """
    class_id = _teacher_class_id(user, db)
    if not _student_in_teacher_class(student_id, class_id, db):
        raise HTTPException(status_code=404, detail="Student not found in your class")
...
def upsert_marks(
    ...
    # Always write the single latest row; prune any older duplicates so every
    # later save / send / read (Admin, Teacher and Parent) sees this one version.
    card = latest_marks_card_pruning_duplicates(db, student_id, academic_year)
    if card:
        card.data = preserve_signatures(card.data, marks_data)
    else:
...
@router.post("/marks/{student_id}/signature")
async def upload_marks_signature(
    ...
    """Upload / replace a signature image on the current marks card for a
    student in the teacher's class."""
    class_id = _teacher_class_id(user, db)
    if not _student_in_teacher_class(student_id, class_id, db):
        raise HTTPException(status_code=404, detail="Student not found in your class")
