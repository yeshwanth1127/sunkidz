...
from app.core.class_names import grade_sort_key
...
def get_dashboard(
    ...
    classes = sorted(
        db.query(Class).filter(Class.branch_id == branch_id).all(),
        key=lambda c: grade_sort_key(c.name),
    )
    return {
...
def get_new_student_form_data(
    ...
    _branch_classes = db.query(Class).filter(Class.branch_id == branch_id).all()
    cls = min(_branch_classes, key=lambda c: grade_sort_key(c.name)) if _branch_classes else None
    return {
        "id": None,
...
def get_student_attendance(
    ...
    # Get student and verify they're in coordinator's branch
...
def update_student_attendance(
    ...
    if not attendance:
        attendance = Attendance(student_id=student_id, date=att_date, status=status, marked_by=user.id)
        db.add(attendance)
    else:
