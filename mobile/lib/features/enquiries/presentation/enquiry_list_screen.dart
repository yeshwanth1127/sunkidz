...
class _EnquiryListScreenState extends ConsumerState<EnquiryListScreen> {
  ...
  /// Grade/class options for the currently selected branch (or, when no
  /// branch is selected, the union across every branch's real classes).
  /// Always derived from actually-configured classes -- never a hardcoded
  /// per-system list -- so a branch only ever offers what it truly has.
  List<String> get _availableGrades {
    final systemTypeByBranch = <String, String?>{
      for (final b in _branches)
        if (b['id'] != null) b['id'].toString(): b['system_type'] as String?,
    };
    final labels = <String>{};
    for (final c in _classes) {
      final branchId = c['branch_id']?.toString();
      final systemType = branchId != null ? systemTypeByBranch[branchId] : null;
      final label = canonicalGradeLabel(c['name']?.toString(), systemType);
      if (label.isNotEmpty) labels.add(label);
    }
    // Fixed Playgroup -> IG1 -> IG2 -> IG3 order, never alphabetical.
    final sorted =
        labels.toList()
          ..sort((a, b) => gradeSortIndex(a).compareTo(gradeSortIndex(b)));
    return sorted;
  }
  ...
  Future<void> _load() async {
    ...
    try {
      final enquiries = await api.getEnquiries(branchId: _selectedBranchId);
      final branches = await api.getBranches();
      final classes = await api.getClasses(branchId: _selectedBranchId);
      if (mounted) {
        setState(() {
  ...
  void _onBranchFilterChanged(String? branchId) {
    setState(() {
      ...
      // The grade set depends on the branch, so a grade chosen under the
      // previous branch may no longer apply.
      _selectedGrade = null;
    });
    _load();
  }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              'Reject enquiry for ${enquiry['child_name'] ?? 'this child'}?',
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Enquiry rejected')));
        await _load();
        ref.invalidate(dashboardDataProvider);
      }
    } catch (e) {
    ...
    return Padding(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButtonFormField<String>(
          ...
          onChanged: _onGradeFilterChanged,
        ),
      ),
    );
  }
...
class _AddEnquirySheetState extends ConsumerState<_AddEnquirySheet> {
  ...
  void dispose() {
    ...
    _fatherWorkCtrl.dispose();
    _motherNameCtrl.dispose();
    _motherPhoneCtrl.dispose();
    _motherEmailCtrl.dispose();
    _motherOccupationCtrl.dispose();
    _motherWorkCtrl.dispose();
    _addressCtrl.dispose();
    _residentialPhoneCtrl.dispose();
    _siblingsInfoCtrl.dispose();
    _siblingsAgeCtrl.dispose();
  ...
  Future<void> _submit() async {
    ...
    if (_childNameCtrl.text.trim().isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Child name is required')));
      return;
    }
    if (_branchId == null) {
      ScaffoldMessenger.of(
    ...
    try {
      final payload = <String, dynamic>{
        ...
        if (_t(_ageYearsCtrl) != null)
          'age_years': int.tryParse(_ageYearsCtrl.text.trim()),
        if (_t(_ageMonthsCtrl) != null)
          'age_months': int.tryParse(_ageMonthsCtrl.text.trim()),
        if (_gender != null) 'gender': _gender,
        if (_t(_fatherNameCtrl) != null) 'father_name': _t(_fatherNameCtrl),
        if (_t(_fatherPhoneCtrl) != null)
          'father_contact_no': _t(_fatherPhoneCtrl),
        if (_t(_fatherEmailCtrl) != null) 'father_email': _t(_fatherEmailCtrl),
        if (_t(_fatherOccupationCtrl) != null)
          'father_occupation': _t(_fatherOccupationCtrl),
        if (_t(_fatherWorkCtrl) != null)
          'father_place_of_work': _t(_fatherWorkCtrl),
        if (_t(_motherNameCtrl) != null) 'mother_name': _t(_motherNameCtrl),
        if (_t(_motherPhoneCtrl) != null)
          'mother_contact_no': _t(_motherPhoneCtrl),
        if (_t(_motherEmailCtrl) != null) 'mother_email': _t(_motherEmailCtrl),
        if (_t(_motherOccupationCtrl) != null)
          'mother_occupation': _t(_motherOccupationCtrl),
        if (_t(_motherWorkCtrl) != null)
          'mother_place_of_work': _t(_motherWorkCtrl),
        if (_t(_addressCtrl) != null) 'residential_address': _t(_addressCtrl),
        if (_t(_residentialPhoneCtrl) != null)
          'residential_contact_no': _t(_residentialPhoneCtrl),
        if (_t(_siblingsInfoCtrl) != null)
          'siblings_info': _t(_siblingsInfoCtrl),
        if (_t(_siblingsAgeCtrl) != null) 'siblings_age': _t(_siblingsAgeCtrl),
        if (_t(_challengesCtrl) != null)
          'challenges_specialities': _t(_challengesCtrl),
        if (_t(_expectationsCtrl) != null)
          'expectations_from_school': _t(_expectationsCtrl),
        'branch_id': _branchId,
        'status': 'pending',
      };
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _f(
                              _dobCtrl,
                              'Date of Birth',
                              ico: Icons.calendar_today_rounded,
                              ro: true,
                              tap: _pickDate,
                            ),
                            Row(
...
class _ConvertEnquirySheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ConvertEnquirySheet> createState() =>
      _ConvertEnquirySheetState();
}
...
class _ConvertEnquirySheetState extends ConsumerState<_ConvertEnquirySheet> {
  ...
  /// Class/Grade options are labeled correctly for that branch.
  ...
  Future<void> _bootstrap() async {
    ...
    _parentNameCtrl.text =
        fatherName.isNotEmpty
            ? fatherName
            : (enq['mother_name']?.toString().trim() ?? '');
    _parentPhoneCtrl.text =
        enq['father_contact_no']?.toString() ??
        enq['mother_contact_no']?.toString() ??
        '';
    ...
    try {
      ...
      if (branchId == null ||
          !branches.any((b) => b['id']?.toString() == branchId)) {
        branchId =
    ...
    try {
      await api.createAdmissionFromEnquiry({
        ...
        if (widget.enquiry['residential_address'] != null)
          'residential_address':
              widget.enquiry['residential_address'].toString(),
        if (widget.enquiry['gender'] != null)
          'gender': widget.enquiry['gender'].toString(),
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Converted to admission — parent can log in with admission number and DOB',
            ),
          ),
        );
        widget.onDone();
