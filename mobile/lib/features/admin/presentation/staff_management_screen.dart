    ...
    try {
      final role =
          _tabIndex == 1
              ...
              ? 'bus_staff'
              : null;
      final users = await api.getUsers(role: role, branchId: _selectedBranchId);
    ...
    showDialog(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              'Are you sure you want to remove ${user['full_name']}? This action is permanent.',
            ),
            actions: [
              TextButton(
...
class _AddUserSheetState extends State<_AddUserSheet> {
  ...
  String _selectedBranchSystemLabel() {
    ...
    final systemType =
        (branch['system_type']?.toString().toLowerCase() ?? 'sunkidz');
  ...
  bool get _needsBranch =>
      widget.role == 'teacher' || widget.role == 'coordinator';
  ...
  Future<void> _submit() async {
    ...
    if (_needsBranch && _branchId == null) {
      setState(() => _error = 'Branch is required');
      return;
    }
  ...
  @override
  Widget build(BuildContext context) {
    final selectedBranchClasses =
        _branchId != null ? _getClassesForBranch(_branchId!) : [];
          ...
          children: [
                ...
                items:
                    widget.branches.map((branch) {
                      return DropdownMenuItem(
                        ...
                        child: Text(
                          branch['name'] as String? ?? 'Unknown Branch',
                        ),
                      );
                    }).toList(),
                onChanged: (value) {
            ...
            if (_needsClass && _branchId != null) ...[
              ...
              DropdownButtonFormField<String>(
                ...
                items:
                    selectedBranchClasses.map((cls) {
                      return DropdownMenuItem(
                        value: cls['id'] as String,
                        child: Text(
                          canonicalGradeLabel(cls['name'] as String?),
                        ),
                      );
                    }).toList(),
                onChanged: (value) {
...
class _ReassignSheetState extends State<_ReassignSheet> {
  ...
  Future<void> _loadClasses(String branchId) async {
    try {
      final classes = sortByCanonicalGrade(
        await widget.api.getClasses(branchId: branchId),
        (c) => c['name'] as String?,
      );
  ...
  Future<void> _submit() async {
    ...
    if (_isTeacher && _classId == null) {
      setState(() => _error = 'Class is required for teachers');
      return;
    }
    setState(() {
      _loading = true;
    ...
    try {
      ...
      final existing =
          assignments
              .where(
                (a) =>
                    a['user_id']?.toString() == widget.user['id']?.toString(),
              )
              .toList();
      if (existing.isEmpty) {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            Text(
              'Reassign ${widget.user['full_name'] ?? 'Staff'}',
              style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800),
            ),
            const SizedBox(height: 24),
            DropdownButtonFormField<String>(
            ...
            if (_classes.isNotEmpty) ...[
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  labelText: _isTeacher ? 'Class *' : 'Class (optional)',
                ...
                items: [
                  ...
                  ..._classes.map(
                    (c) => DropdownMenuItem(
                      value: c['id']?.toString(),
                      child: Text(canonicalGradeLabel(c['name']?.toString())),
                    ),
                  ),
                ],
