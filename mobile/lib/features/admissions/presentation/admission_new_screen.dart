...
class _AdmissionNewScreenState extends ConsumerState<AdmissionNewScreen> {
  ...
  void dispose() {
    for (final c in [
      ...
      _parentContactCtrl,
      _prevSchoolNameCtrl,
      _prevSchoolDurationCtrl,
      _prevSchoolClassCtrl,
      _parentSearchCtrl,
    ]) {
      c.dispose();
    }
    ...
    try {
      ...
      if (mounted)
        setState(() {
          _classes = classes;
          _classId =
              classes.isNotEmpty ? classes.first['id']?.toString() : null;
        });
    } catch (_) {
      if (mounted)
        setState(() {
  ...
  Future<void> _submit() async {
    ...
    if (_branchId == null || _classId == null) {
      _snack('Branch and class are required');
      return;
    }
    if (_nameCtrl.text.trim().isEmpty) {
      _snack('Student name is required');
      return;
    }
    ...
    if (_parentNameCtrl.text.trim().isEmpty) {
      _snack('Parent name (for login) is required');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Admission created successfully! Parent can login with Admission No & DOB.',
            ),
          ),
        );
                  ...
                  children: [
                      ...
                      if (_parentUserId != null)
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                            ...
                            hintText:
                                'Enter at least 3 digits to find existing data',
                        ...
                        const Text(
                          'Or enter new parent details below:',
                          style: TextStyle(fontSize: 12, color: Colors.grey),
                        ),
                        const SizedBox(height: 8),
                        _field(
                          _parentNameCtrl,
                          'Parent Name (for login) *',
                          Icons.person,
                        ),
                        _field(
                          _parentContactCtrl,
                    ...
                    _section('Previous Schooling', Icons.history_edu_rounded, [
                      SwitchListTile(
                        ...
                        onChanged:
                            (v) => setState(() => _attendedPreviously = v),
                    ...
                    _section('Documents Submitted', Icons.folder_rounded, [
                      _check(
                        'Birth Certificate',
                        _birthCert,
                        (v) => setState(() => _birthCert = v!),
                      ),
                      _check(
                        'Immunization Record',
                        _immunization,
                        (v) => setState(() => _immunization = v!),
                      ),
                      _check(
                        'Transfer Certificate',
                        _transferCert,
                        (v) => setState(() => _transferCert = v!),
                      ),
                      _check(
                        'Passport Photos',
                        _passportPhotos,
                        (v) => setState(() => _passportPhotos = v!),
                      ),
                      _check(
                        'Progress Report',
                        _progressReport,
                        (v) => setState(() => _progressReport = v!),
                      ),
                      _check(
                        'Passport Copy',
                        _passport,
                    ...
                    FilledButton.icon(
                      ...
                      label: Text(
                        _submitting ? 'Processing...' : 'Complete Admission',
                        style: const TextStyle(
