...
class CoordinatorAttendanceScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CoordinatorAttendanceScreen> createState() => _CoordinatorAttendanceScreenState();
}

class _CoordinatorAttendanceScreenState extends ConsumerState<CoordinatorAttendanceScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAttendance() async {
    ...
    try {
      final res = await api.getAttendance(date: _dateStr(_selectedDate), classId: _filterClassId);
      setState(() {
        _attendanceData = res;
        _loading = false;
      });
  ...
  Future<void> _loadHistory() async {
    ...
    setState(() => _loading = true);
    try {
      final res = await api.getAttendanceHistory(period: _historyPeriod, classId: _filterClassId);
      setState(() {
        _historyData = res;
        _loading = false;
      });
        ...
        child: Column(
          ...
          children: [
            ...
            if (_loading && _attendanceData == null)
              ...
              _buildByClassContent(_attendanceData!['by_class'] as Map<String, dynamic>? ?? {}),
          ],
        ),
      ),
    );
          ...
          return Column(
            ...
            children: [
              Padding(
                ...
                child: Text(className, style: Theme.of(context).textTheme.titleSmall?.copyWith(color: AppColors.primaryLight)),
              ),
                        ...
                        child: Wrap(
                          ...
                          children: dates.reversed.map((dk) {
                            ...
                            return Container(
                              ...
                              child: Text('${_formatDate(dk)}: ${(status as String).toUpperCase()}', style: TextStyle(fontSize: 12, color: c, fontWeight: FontWeight.w500)),
                            );
                          }).toList(),
                        ),
                      ),
